...
- **Store Locations** — add, remove, and edit stores. Each store has a name, address, city, state, zip, and phone. The address shows up on every invoice PDF as the "Ship To" block. Includes a one-click "Import Default McCoy's Store List" button that loads all 93 stores from the master list.
