...
async function deletePart(partId) {
  ...
  try {
    ...
    pruneDeletions().catch(err => console.error("Prune deletions failed:", err));
...
// Keep only the most recent 10 deletions
async function pruneDeletions() {
  const snap = await db.collection("deletions").orderBy("deletedAt", "desc").get();
  ...
  body.innerHTML = adminState.deletions.map(d => {
    ...
    return `
      <tr>
        ...
        <td>${summary}</td>
        <td>
          <button data-restore="${escapeHtml(d.id)}">Restore</button>
        </td>
      </tr>
    `;
  }).join("");
  ...
  try {
    ...
    for (const [partId, requested] of requestedByPart.entries()) {
      ...
      batch.update(db.collection("parts").doc(partId), {
        currentQuantity: Number(part.currentQuantity || 0) - requested
      });
    }
    ...
    const restored = Object.assign({}, snap, {
      ...
      // We don't carry over the editingBy/editingSince lock if it existed
