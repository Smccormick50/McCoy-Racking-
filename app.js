...
function attachListeners() {
  ...
  db.collection("settings").doc("locations").onSnapshot(doc => {
    if (doc.exists && Array.isArray(doc.data().list)) {
      // Normalize: accept either strings (old format) or full objects (new).
      // Always end up with an array of { name, phone?, address?, city?, state?, zip? }.
...
// Stored ISO dates like "2026-05-21" -> "2026-05"
// Older locale-formatted dates ("5/21/2026") get parsed best-effort.
function invoiceMonthKey(inv) {
  const d = inv && inv.date ? String(inv.date) : "";
  // ISO date
  const iso = /^(\d{4})-(\d{2})-\d{2}$/.exec(d);
  if (iso) return `${iso[1]}-${iso[2]}`;
  // M/D/YYYY locale date
  const us = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(d);
  if (us) return `${us[3]}-${String(us[1]).padStart(2, "0")}`;
  // Anything else: try Date parsing as a last resort
...
async function useInventory(event) {
  ...
  try {
    ...
    if (isEditing) {
      ...
      const locDetails = stripUndefined(getLocationByName(locName) || { name: locName });
    ...
    const savedInvoiceLoc = qs("location").value;
    const savedInvoice = {
      invoiceNumber,
      date: isEditing
      ...
      location: savedInvoiceLoc,
      locationDetails: getLocationByName(savedInvoiceLoc) || { name: savedInvoiceLoc },
      user,
      lineItems: invoiceLines,
      total: invoiceLines.reduce((s, l) => s + Number(l.total || 0), 0)
    };
...
function buildAndDownloadInvoicePdf(invoice) {
  ...
  // Brand colors (RGB)
  const GREEN_DARK   = [15, 58, 42];     // #0f3a2a
  const GREEN_MID    = [22, 107, 70];    // #166b46
  const YELLOW       = [245, 209, 22];   // #f5d116
  ...
  // Yellow accent stripe under the band
  doc.setFillColor(...YELLOW);
  doc.rect(0, 32, PAGE_W, 2, "F");

  // Brand text: "Mc Racking" big in yellow on the left
  ...
  // ===== INVOICE METADATA BLOCK (below header) =====
  doc.setTextColor(...GREEN_DARK);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(15);
  doc.text("RACKING INVENTORY INVOICE", 14, 46);

  // Left column: invoice number + date + edited
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  ...
  // Right column: "Ship To" box with store address
  const shipBoxX = 115;
  const shipBoxY = 50;
  const shipBoxW = 81;
  const shipBoxH = 38;
  ...
  // User row below the metadata
  let belowY = Math.max(82, shipBoxY + shipBoxH + 4);
  if (invoice.user) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.setTextColor(80, 80, 80);
